# CSUM-08 Business Banking Approval Settings

**SUMMERVILLE CREDIT UNION · BUSINESS BANKING USER GUIDE · CSUM-08 of 16**
...
Approval Settings enables business administrators to configure dual-control requirements for different transaction types. The feature allows the business to specify how many approvals are required before a transaction is processed, supporting values from 1 (auto-approved) to multiple approvers for high-risk transaction types.

The settings screen displays each transaction type (ACH Payment, ACH Collection, ACH Payroll, Domestic Wires) with a dropdown to set the required number of approvals. When set to 1, transactions initiated by an approver are auto-approved. Higher values enforce multi-party authorization before any payment is released.

For credit unions, approval settings provide the dual-control framework that regulators expect for commercial banking operations. This feature directly addresses NCUA examination requirements for business account controls and helps credit unions demonstrate adequate safeguards for commercial payment processing.
...
| **Access Level**         | Admin-only; requires Approval Settings permission      |
| **Key Actions**          | Set approval thresholds per transaction type           |
| **Regulatory Relevance** | Dual-control compliance, NCUA examination requirements |
...
| ---------------------------- | --------------- | -------------------------------------------------- | ---------------------------------------------------------- |
| Configure Dual-Control       | Business Owner  | Set wire transfers to require 2 approvals          | Enforces dual-control for high-value, irrevocable payments |
| Streamline Low-Risk Payments | Business Admin  | Set ACH payments to 1 approval for auto-processing | Reduces friction for routine, low-risk transactions        |
| Payroll Controls             | Business Owner  | Require 2 approvals for payroll batch processing   | Additional oversight for bulk employee payments            |
| Audit Configuration          | Business Admin  | Review current approval settings across all types  | Ensures settings align with business risk policy           |

|                                |
| ------------------------------ |
| **03 STEP-BY-STEP USER GUIDE** |

**How to get here: Dashboard → Business Banking → Approval Settings**
...
![Figure 2 — Open the Business Banking Hub](<../../../.gitbook/assets/image3 (3).png>)
...
![Figure 3 — Navigate to Approval Settings](<../../../.gitbook/assets/image2 (3).png>)

_Figure 3 — Navigate to Approval Settings_
...
![Figure 4 — Configure Thresholds and Save](<../../../.gitbook/assets/image4 (3).png>)

_Figure 4 — Configure Thresholds and Save_
