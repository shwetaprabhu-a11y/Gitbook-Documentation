...
The Custom Account Grouping feature within the nFinia digital banking platform enables members to organize their accounts into personalized groups and sections on the Accounts Overview screen. By default, the platform displays accounts in system-defined groups such as Deposit Accounts and Loan Accounts. With Custom Account Grouping, members can create their own groups (e.g., \'Family Savings,\' \'Business Accounts,\' \'Vacation Fund\'), add sections within groups for further categorization, move accounts between groups, and reorder groups to match their personal financial management preferences.

This feature serves all digital banking members --- from individual consumers managing personal finances to business owners organizing multiple operating and reserve accounts. For credit unions, Custom Account Grouping delivers a highly personalized self-service experience that increases member engagement with the platform, reduces friction in account navigation, and differentiates the digital banking offering from competitors that rely on rigid, system-defined account structures.

The feature is accessible from the Accounts Overview screen via the Account Group dropdown. Members can switch between the default \'All Accounts\' view and any custom groups they have created. The Manage Groups interface provides full control over group creation, editing, deletion, section management, account assignment, and group sort order. Released in nFinia version 10.44, this feature is part of the platform\'s ongoing personalization strategy.
...
**2. Use Cases**
  ...
  **Use Case**                                    **Who Uses It**          **What They Do**                                                                                                                      **Business Value**
  ...
  Separate business and personal accounts         Business Owner           Creates a \'Business Accounts\' group to isolate operating, payroll, and reserve accounts from personal accounts                      Reduces operational confusion for business members managing multiple account types within a single profile
  ...
  Organize accounts with sections within groups   Business Owner / Admin   Creates sections within a group (e.g., \'Operating\' and \'Reserve\' sections within \'Business Accounts\') for sub-categorization    Provides multi-level organization for members with complex account portfolios, reducing navigation complexity
  ----------------------------------------------- ------------------------ ------------------------------------------------------------------------------------------------------------------------------------- -----------------------------------------------------------------------------------------------------------------------------

These use cases highlight how Custom Account Grouping transforms the Accounts Overview from a static, system-defined list into a personalized financial dashboard. For Summerville FCU, this personalization capability is a key differentiator that increases digital banking stickiness, reduces the likelihood of members switching to competitor platforms, and demonstrates the credit union\'s commitment to member-centric design.
...
\- If the member has no custom groups, the Account Group dropdown shows only system-defined groups and the \'Manage Groups\' option.

\- If the member attempts to delete a custom group that contains accounts, the system prompts for confirmation and moves accounts back to the default \'All Accounts\' view.

\- System groups cannot be deleted or renamed, only their internal sections and account arrangement can be modified.

\- If the member creates a section within a group, accounts can be moved between sections via drag-and-drop or the Move Account action.
...
Upon saving a new or edited group, the system persists the configuration and returns the member to the Accounts Overview with the updated grouping applied. The Account Group dropdown immediately reflects the new or modified group. No notifications or audit log entries are generated as this is a presentation-layer personalization feature with no transactional impact.
...
\- Session timeout during editing: Unsaved changes are lost; the member is redirected to login. The system does not auto-save partial group configurations.
...
The Accounts Overview screen is the primary landing page for account information. The Account Group dropdown at the top allows members to filter their view by group. This is the entry point for all account grouping interactions.
...
This form allows members to create a new custom account group. It provides fields for naming the group, creating internal sections, and assigning accounts to the group.
...
This view allows members to customize the internal structure of system-defined groups. While the group name and core accounts are system-managed, members can add sections and rearrange accounts within the group.

  --------------------- ------------------- ------------------------------------------------------------------------------ ----------------------------------------------
  **Field / Element**   **Type**            **Description**                                                                **Notes**
  Group Name            Label (read-only)   System-defined group name (e.g., \'Deposit Accounts\')                         Cannot be edited by member
  Sections              Expandable List     Shows existing sections within the system group with their assigned accounts   Members can add new sections
...
This interface allows members to control the display order of all account groups in the Account Group dropdown and on the Accounts Overview screen.
...
**5. Quick Reference**
  ...
  **Task**                          **Navigation Path**                                                       **Who Can Do It**   **Notes**
  View account groups               Accounts Overview \> Account Group Dropdown                               All Members         Dropdown shows system + custom groups
  Create a custom group             Accounts Overview \> Manage Groups \> New Custom Group                    All Members         Enter name, add sections, assign accounts
  Edit a custom group               Accounts Overview \> Manage Groups \> \[Group\] \> Edit                   All Members         Rename, add/remove sections and accounts
  Delete a custom group             Accounts Overview \> Manage Groups \> \[Group\] \> Edit \> Delete Group   All Members         Accounts return to default view; requires confirmation
  Edit a system group               Accounts Overview \> Manage Groups \> \[System Group\] \> Edit            All Members         Add sections only; cannot rename or delete system groups
  Add accounts to a group           Edit Group \> Add Account Dropdown                                        All Members         Select from available accounts; accounts can belong to multiple groups
  Sort/reorder groups               Accounts Overview \> Manage Groups \> Sort                                All Members         Drag-and-drop to set display order
  Create a section within a group   Edit Group \> Add Section                                                 All Members         Sub-categorize accounts within a group
  --------------------------------- ------------------------------------------------------------------------- ------------------- ------------------------------------------------------------------------

**6. Conclusion**

The Custom Account Grouping feature delivers a personalized account management experience for Summerville FCU members, transforming the Accounts Overview from a static, system-defined list into a flexible, member-controlled financial dashboard. By enabling custom groups, sections, account assignment, and sort ordering, the platform empowers members to organize their financial lives in ways that match their personal or business needs.
