...
Changing a User ID requires the new ID to be unique across the platform and may require OTP verification. Password changes require entry of the current password for verification. All credential changes are logged as security events and trigger a notification to you's registered contact.
...
| Remove Old MFA Method        | You with an old phone number in MFA                                      | Open MFA Settings, select old method, remove after verifying current method | Keeps authentication methods current with you's active devices                                                                                                                                             |
