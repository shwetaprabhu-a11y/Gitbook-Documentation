...
Union to send targeted alerts and messages directly to members mobile
...
push delivery through the Prisma Admin Portal) and end-user members (who
...
| User Roles           | FI Operations (Prisma Admin), Mobile Banking Members            |
...
| Prisma Push Configuration     | FI Operations / Admin          | Configures the Tyfone delivery channel in Prisma with Base URL, Client ID, and credentials; enables tokenized customer identification | One-time setup enables all push notification campaigns across the member base                    |
| Test Push Notification        | FI Operations / QA             | Sends a test push notification to a specific member Individual ID from the Prisma Admin Portal to validate end-to-end delivery        | Verifies push pipeline before production rollout; prevents failed notifications reaching members |
...
alerts to Summerville FCU members.
...
| 7        | Test Push Notification Modal | Enter the member Unique Identifier (Individual ID) and review pre-populated message | Message field shows: "Prisma: Summerville Push Notification Test {system.current-date}"   |
...
password are valid, (c) the member has the mobile app installed with
...
Upon successful test delivery, the member device displays the push
...
| Invalid Individual ID      | Test push dispatched but no recipient matched       | Verify the member Individual ID in the core banking system; ensure the member has an active mobile enrollment |
...
| ------------------- | ------------ | ----------------------------------------------------- | -------------------------------------------------------------------------- |
| **Field / Element** | **Type**     | **Description**                                       | **Notes**                                                                  |
| Unique Identifier   | Text Input   | The member Individual ID to receive the test push     | Must match an active member with mobile app enrolled                       |
...
| Send Test           | Button       | Dispatches the test notification to the member device | Triggers real-time delivery through Tyfone channel                         |
...
When push notifications are delivered to the member device, they appear
...
| Notification Centre  | Notifications are accessible from the lock screen or by swiping down from any screen         | Member can tap any notification to open the Summerville FCU mobile app directly |
